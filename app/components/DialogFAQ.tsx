...
const FAQ_ENTRIES: Array<{ question: string; answer: string }> = [
  {
    question: "How do I...?",
    answer:
      "<strong>Products</strong><ul class='faq-list'>" +
      ...
      "<li><strong>Sync everything at once</strong> — use the <strong>Sync All</strong> button at the top of the Products page. This will sync any products that have changes and you'l be asked if you also want to publish any products that have never been published to the site.</li></ul>" +
      "<strong>Inventory</strong><ul class='faq-list'><li><strong>Adjust Inventory</strong> — Navigate to the <a href='/inventory'>Inventory page</a> and adjust the stock quantity for a simple product or a variant. <p class='margin-bs-half'>If a product has:</p><ul> <li><strong class='clr-danger'>Conflicts</strong> (the warehouse stock differs from the stock currently on the website) as indicated by <span class='clr-danger dot'></span>. You can resolve them by choosing the 'resolve conflicts' mode.</li><li><strong class='clr-warning'>Out of Stock</strong> (the website stock is 0) as indicated by <span class='clr-warning dot'></span>. You can adjust the warehouse stock quantity and sync changes.</li><li><strong class='clr-accent'>New changes</strong> (the warehouse stock has been altered) as indicated by <span class='clr-accent dot'></span>. You can choose to sync with 'sync changes' mode to only push these products, or with mode 'custom select' and your changes are automatically included in the selection.</li></ul> </li>" +
      "</ul>",
  },
  {
    question:
    ...
    answer:
      ...
      "<li><strong>Status</strong> / <strong>Publish to site</strong> (on the product card) — the only button that changes Draft ⇄ Published. This is a separate, deliberate choice from editing content.</li>" +
      "</ul>" +
